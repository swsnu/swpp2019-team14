...
class CurationAddBookModal extends Component {
  state = {
    searchKeyword: '',
    searchedBookList: [],
    book: '',
    ...
    const ADD_BOOK = this.props.show && (
      <div id="add-book-modal">
        ...
        <Button
          ...
          content="Search"
          onClick={() => this.onClickSearchBookButton()}
        />
        {bookList}
        <Button
          id="choose-book-button"
          content="Choose"
          onClick={() => this.onClickChooseBookButton()}
        />
      </div>
    );
    return ADD_BOOK;
